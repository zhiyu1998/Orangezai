...
export class BrowserManager {
    ...
    configure(config = {}) {
        ...
        this.deviceScaleFactor = this.normalizePositiveNumber(
            config.deviceScaleFactor,
            this.defaultConfig.deviceScaleFactor
        )
        this.browserTimeout =
    ...
    async getScreenshotTarget(page) {
        const container = page.locator("#container").first()
        if (await container.count()) {
            return container
        }

        const fallbackAttr = "data-orangezai-screenshot-target"
        const hasSingleContentRoot = await page.evaluate(attr => {
            ...
            candidates[0].setAttribute(attr, "true")
            return true
        }, fallbackAttr)

        if (hasSingleContentRoot) {
            return page.locator(`body > [${fallbackAttr}="true"]`).first()
        }
    ...
    async screenshot(name, data = {}, hooks = {}) {
        ...
        try {
            ...
            const target = await this.getScreenshotTarget(page)
